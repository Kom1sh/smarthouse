...
import { Sensor, Notification } from "@/lib/types";
...
function getOverallStatus(sensors: Sensor[], notifications: Notification[]): {
  ...
  if (criticalSensors.length > 0 || unreadCritical.length > 0) {
    return {
      ...
      subtitle: `${criticalSensors.length} датчик(ов) в критическом состоянии. Проверьте уведомления.`,
    };
  }
  if (warningSensors.length > 0) {
    return {
      ...
      subtitle: `${warningSensors.length} датчик(ов) с предупреждениями. Ситуация под контролем.`,
    };
  }
  return {
    status: "ok",
    title: "Дом в порядке",
    subtitle: "Все системы работают штатно. Показатели в пределах нормы.",
  };
}
...
export default function HomeStatusBanner({ sensors, notifications }: HomeStatusBannerProps) {
  const { status, title, subtitle } = getOverallStatus(sensors, notifications);
  const c = cfg[status];
  const Icon = c.icon;
  ...
  return (
    <div className={cn("rounded-2xl bg-gradient-to-r text-white p-5 shadow-md", c.gradient)}>
      <div className="flex items-start gap-4">
        <div className={cn("w-12 h-12 shrink-0 rounded-xl bg-white/20 flex items-center justify-center ring-4 backdrop-blur-sm", c.ring)}>
